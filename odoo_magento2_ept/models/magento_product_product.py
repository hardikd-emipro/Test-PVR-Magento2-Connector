...
class MagentoProductProduct(models.Model):
    ...
    def search_product_in_layer(self, line, item):
        ...
        if 'is_order' not in list(self.env.context.keys()):
            if not line.do_not_update_existing_product and prices:
        ...
        if not template:
            ...
            if not template:
                ...
                if is_verify:
                    ...
                    return is_verify
        if line.instance_id.auto_create_product:
            m_template.create_configurable_template(line, item, template)
        else:
    ...
    def get_products(self, instance, ids, line):
        filters = create_search_criteria({'entity_id': {'in': ids}})
        query_str = Php.http_build_query(filters)
        url = f"/V1/products?{query_str}"
        response = {}
        try:
    ...
    def export_multiple_product_stock_to_magento(self, instance):
        ...
        if stock_data:
            ...
            job = self.exp_prd_stock_in_batches(stock_data, instance, api_url, 'skuData', 'PUT',
                                                job)
    ...
    def export_product_stock_to_multiple_locations(self, instance, magento_locations):
                ...
                if export_product_stock:
                    for product_id, stock in export_product_stock.items():
                        stock_data = self.prepare_export_product_stock_dict(
                            product_id, instance, stock, consumable_products, stock_data,
                            magento_location)
            else:
                raise UserError(
        ...
        if stock_data:
            ...
            job = self.exp_prd_stock_in_batches(stock_data, instance, api_url, 'sourceItems',
                                                'POST', job)
        if not job.log_lines:
            job.sudo().unlink()
        return True
    ...
    def exp_prd_stock_in_batches(self, stock_data, instance, api_url, data_key, method, job):
        ...
        for page in range(current_page, total_stock_dict_pages + 1):
            stock_dict = [stock_data[x] if stock_data else [] for x in
