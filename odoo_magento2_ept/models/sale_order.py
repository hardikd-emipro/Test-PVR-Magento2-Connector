...
class SaleOrder(models.Model):
    ...
    def __update_partner_dict(self, item, instance):
        ...
        item.update({
            ...
            'website_id': magento_store.magento_website_id.id,
        ...
        item = self.__update_partner_address_dict(item, item.get('billing_address'))
        ship_assign = item.get('extension_attributes', dict()).get('shipping_assignments')
        for ship_addr in ship_assign:
            if isinstance(ship_addr, dict) and 'address' in list(ship_addr.keys()):
                ship_addr = ship_addr.get('shipping', dict()).get('address')
                item = self.__update_partner_address_dict(item, ship_addr)
        return item
    ...
    def __update_partner_address_dict(item, addresses):
        item['addresses'].append({
            ...
            'default_billing': addresses.get('address_type') == 'billing',
            'default_shipping': addresses.get('address_type') == 'shipping',
        })
        return item
