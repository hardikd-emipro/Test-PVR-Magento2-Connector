...
class MagentoProductTemplate(models.Model):
    ...
    def create_configurable_template(self, line, item, template):
        ...
        self._create_configurable_variant(line, item, template, data)
    ...
    def get_product_images(self, item, data, line):
        ...
        if base_url:
            ...
            for image in item.get('media_gallery_entries', list()):
                image_binary = False
                try:
                    image_binary = common_image.get_image_ept(image.get('full_path', ''))
    ...
    def __set_price_list_item(product, price, website):
        price_lists = website.pricelist_ids.filtered(lambda p: p.currency_id.name == price.get('default_store_currency'))
    ...
    def prepare_export_stock_data(self, product_stock, instance, log, source_code=False, msi=False):
        """
        ...
        for product_id, stock in product_stock.items():
            product = m_product.search([('odoo_product_id', '=', product_id),
                                        ('magento_instance_id', '=', instance.id)], limit=1)
            if product and stock >= 0.0:
                if product.odoo_product_id.type != 'product':
